...
param_details = {
    ...
    "BarConvTol": {
        "description": "The barrier solver terminates with a \"GRB_OPTIMAL\" status when some\nsolution quality attributes are less than the specified tolerance. The\nalgorithm considers the relative difference between the primal and\ndual objective values, relative primal and dual feasibility, and\ncomplementarity. Tightening this tolerance often produces a more\naccurate solution, which can sometimes reduce the time spent in\ncrossover.  Be aware that such tightening may result in an increase of\nbarrier iterations and hence computation time spent therein.\nLoosening it causes the barrier algorithm to terminate with a less\naccurate solution, which can be useful when barrier is making very\nslow progress in later iterations but increases chances of prolonged\nruntime in crossover.\n\nThis parameter does not affect models with quadratic constraints. For\nthese models use BarQCPConvTol.\n\nNote:\n\n  Barrier only",
        "name": "BarConvTol",
        "values": {
            "default": 1e-08,
            "maximum": 1.0,
    ...
    "BarQCPConvTol": {
        "description": "When solving a QCP model, the barrier solver terminates with a\n\"GRB_OPTIMAL\" status when some solution quality attributes are less\nthan the specified tolerance. The algorithm considers the relative\ndifference between the primal and dual objective values, relative\nprimal and dual feasibility, and complementarity.  Tightening this\ntolerance may lead to a more accurate solution, but it may also lead\nto a failure to converge.\n\nNote:\n\n  Barrier only",
        "name": "BarQCPConvTol",
        "values": {
            "default": 1e-06,
            "maximum": 1.0,
    ...
    "FuncPieceLength": {
        "description": "Deprecated since version 13.0: Using function constraints is\ndeprecated since version 13.0. Please use nonlinear constraints\ninstead.\n\nIf the FuncPieces parameter is set to value 1, this parameter gives\nthe length of each piece of the piecewise-linear approximation.",
    ...
    "ImproveStartNodes": {
        "description": "The MIP solver can change parameter settings in the middle of the\nsearch in order to adopt a strategy that gives up on moving the best\nbound and instead devotes all of its effort towards finding better\nfeasible solutions. This parameter allows you to specify the node\ncount at which the MIP solver switches to this solution improvement\nstrategy. For example, setting this parameter to 10 will cause the MIP\nsolver to switch strategies once the node count is larger than 10,\nprovided that at least one feasible solution has been found. If no\nincumbent solution exists when the specified node count is reached,\nthe strategy switch will occur as soon as the first feasible solution\nis discovered.\n\nNote:\n\n  Only affects mixed integer programming (MIP) models",
    ...
    "ImproveStartTime": {
        "description": "The MIP solver can change parameter settings in the middle of the\nsearch in order to adopt a strategy that gives up on moving the best\nbound and instead devotes all of its effort towards finding better\nfeasible solutions. This parameter allows you to specify the time (in\nseconds) when the MIP solver switches to this solution improvement\nstrategy. For example, setting this parameter to 10 will cause the MIP\nsolver to switch strategies 10 seconds after starting the\noptimization, provided that at least one feasible solution has been\nfound. If no incumbent solution exists when the specified time is\nreached, the strategy switch will occur as soon as the first feasible\nsolution is discovered.\n\nNote:\n\n  Only affects mixed integer programming (MIP) models",
    ...
    "ImproveStartWork": {
        "description": "The MIP solver can change parameter settings in the middle of the\nsearch in order to adopt a strategy that gives up on moving the best\nbound and instead devotes all of its effort towards finding better\nfeasible solutions. This parameter allows you to specify the work (in\nwork units) when the MIP solver switches to this solution improvement\nstrategy. For example, setting this parameter to 10 will cause the MIP\nsolver to switch strategies 10 work units after starting the\noptimization, provided that at least one feasible solution has been\nfound. If no incumbent solution exists when the specified work is\nreached, the strategy switch will occur as soon as the first feasible\nsolution is discovered.\n\nNote:\n\n  Only affects mixed integer programming (MIP) models",
    ...
    "NLBarCFeasTol": {
        "description": "Important:\n\n  We consider this feature a *preview* in this release. This means\n  that it is fully tested and supported, but will likely undergo\n  significant changes in subsequent Gurobi technical or major\n  releases, potentially including breaking changes in API, behavior\n  and packaging.\n\nFor the NL barrier algorithm, the complementarity error must be\nsmaller than NLBarCFeasTol in order for a model to be declared locally\noptimal. Due to problem transformations like presolve or internal\nscaling, the returned solution\u2019s residuals may deviate from those\nobserved by the algorithm.\n\nNote:\n\n  Only affects the NL barrier algorithm",
        "name": "NLBarCFeasTol",
        "values": {
            "default": 1e-08,
            "maximum": 0.1,
    ...
    "ObjNumber": {
        "description": "When working with multiple objectives, this parameter selects the\nindex of the objective you want to work with. When you query or modify\nan attribute associated with multiple objectives (ObjN, ObjNWeight,\netc.), the ObjNumber parameter determines which objective function is\nactually affected. When you query an attribute associated with multi-\nobjective passes (ObjPassNStatus, ObjPassNObjVal, etc.) and the\nparameter ObjPassNumber is set to -1, the ObjNumber parameter\ndetermines for which pass the value is returned. The value of this\nparameter should be less than the value of the NumObj attribute (which\ncaptures the number of objectives in the model).\n\nPlease refer to the discussion of Multiple Objectives for more\ninformation on the use of alternative objectives.",
        "name": "ObjNumber",
        "values": {
            "default": 0,
            "maximum": 2000000000,
    ...
    "ObjPassNumber": {
        "description": "After a multi-objective model has been solved, this parameter selects\nthe index of the optimization pass for which you want to query results\nsuch as ObjPassNIterCount, ObjPassNMipGap, etc. The value of this\nparameter should be strictly less than the value of the NumObjPasses\nattribute (which captures the number of optimization passes processed\nin the last run). If the parameter value is set to -1 (default), the\noptimization pass selected is the one in which objective k was\nprocessed. You set k using the ObjNumber parameter.\n\nPlease refer to the discussion of Multiple Objectives for more\ninformation on the use of alternative objectives.",
        "name": "ObjPassNumber",
        "values": {
            "default": -1,
            "maximum": 2000000000,
    ...
    "PoolGapAbs": {
        "description": "Determines how large an (absolute) gap to tolerate in stored\nsolutions. When this parameter is set to a non-default value,\nsolutions whose objective values exceed that of the best known\nsolution by more than the specified (absolute) gap are discarded. For\nexample, if the MIP solver has found a solution at objective 100, then\na setting of \"PoolGapAbs=20\" would discard solutions with objective\nworse than 120 (assuming a minimization objective).\n\nNote:\n\n  Only affects mixed integer programming (MIP) models",
        "name": "PoolGapAbs",
        "values": {
            "default": 1e+100,
            "maximum": 1e+100,
    ...
    "PoolSearchMode": {
        "description": "Selects different modes for exploring the MIP search tree. With the\ndefault setting (\"PoolSearchMode=0\"), the MIP solver tries to find an\noptimal solution to the model. It keeps other solutions found along\nthe way, but those are incidental and typically only the improving\nones. By setting this parameter to a non-default value, the MIP search\nwill continue after the optimal solution has been found in order to\nfind additional, high-quality solutions. With a non-default value\n(\"PoolSearchMode=1\" or \"PoolSearchMode=2\"), the MIP solver will try to\nfind \"n\" solutions, where \"n\" is determined by the value of the\nPoolSolutions parameter. With a setting of 1, there are no guarantees\nabout the quality of the extra solutions, while with a setting of 2,\nthe solver will find the \"n\" best solutions. The cost of the solve\nwill increase with increasing values of this parameter.\n\nOnce optimization is complete, the PoolObjBound attribute can be used\nto evaluate the quality of the solutions that were found. For example,\na value of \"PoolObjBound=100\" indicates that there are no other\nsolutions with objective better than 100, and thus that any known\nsolutions with objective better than 100 are better than any as-yet\nundiscovered solutions.\n\nSee Solution Pool for more information about solution pools, including\nsubtleties and limitations.\n\nNote:\n\n  Only affects mixed integer programming (MIP) models",
        "name": "PoolSearchMode",
        "values": {
            "default": 0,
            "maximum": 2,
    ...
    "PoolSolutions": {
        "description": "Determines how many MIP solutions are stored. For the default value of\nPoolSearchMode, these are typically just the improving solutions that\nare found along the way in the process of exploring the MIP search\ntree. For other values of PoolSearchMode, this parameter sets a target\nfor how many solutions to find, so larger values will impact\nperformance.\n\nNote:\n\n  Only affects mixed integer programming (MIP) models",
        "name": "PoolSolutions",
        "values": {
            "default": 10,
            "maximum": 2000000000,
